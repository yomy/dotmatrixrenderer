...
class SmallSquare extends PNGRenderer {
    ...
    protected function renderMatrix() {
        ...
        $squareOffset = floor($this->getModuleSize() * 0.7);
        for ($row = 0; $row < $this->getMatrix()->getSize(); $row++) {
            for ($col = 0; $col < $this->getMatrix()->getSize(); $col++) {
                if ($this->getMatrix()->isDark($row, $col)) {
                    ...
                    imagefilledrectangle($this->GDResource, $x, $y, $x + $squareOffset, $y + $squareOffset, $this->darkColorGD);
                }
            }
        }
    }
